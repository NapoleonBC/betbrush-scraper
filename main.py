...
if __name__ == '__main__':
    try:
        # url set
        # https://www.betrush.com/tipster,1976.html
        url = input("Enter URL: ")
        ...
        logging.error(e)
        logging.error("Please contact Patrick asap:)")
        driver.quit()
        sys.exit(1)
    while True:
        try:
            ...
            logging.warning("Failed to load tipstername, maybe due to the network condition or the site was down")
            continue
        ...
        try:
            ...
            for idx, td_element in enumerate(td_elements):
                ...
                outer_html = trs[i + 1].find_elements(By.CSS_SELECTOR, "table.picks tbody tr")
                outer_html = outer_html[idx].get_attribute('outerHTML')
    ...
    '''
            ...
            logging.warning(e)
            logging.warning("Continue looking up:)")
